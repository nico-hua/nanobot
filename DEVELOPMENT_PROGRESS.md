...
## 最新同步 SpawnTool（2026-09-03）

- [x] 新增内置 `SpawnTool`：模型参数仅为必填 `task`，同步调用 `SubagentManager.run()`；子 Agent 成功后，其最终文本以普通 `ToolResult` 返回主 Agent 并继续现有工具调用循环。
- [x] `SpawnTool` 仅在 `ToolContext.subagent_manager` 存在时启用；没有 Manager、缺少当前 `RequestContext`、任务为空或子 Agent 执行失败时，均返回清晰的工具错误。`asyncio.CancelledError` 保持向上传播。
- [x] Spawn 会把当前请求的 `RequestContext` 传给 `SubagentManager`；Manager 在调用子 `AgentRunner` 的作用域中重新绑定该 `ContextVar`，因此子 Agent 的工具可安全继承当前 session / channel / chat / sender 等路由信息，workspace 仍来自子 Agent 初始化时的独立 `ToolContext`。
- [x] `ToolLoader` 会在主 Agent 的 context 中注册 `SpawnTool`，但子 Agent 的基础 context 不含 `subagent_manager`，所以不会把 Spawn 再加载到子 Agent 的独立 `ToolRegistry`。
- [x] 新增 Spawn 与 Subagent focused tests，覆盖 schema、工厂创建、请求上下文和 workspace 传递、主会话历史隔离、独立工具注册表、成功结果、失败结果与无真实外部服务运行。
- [x] 最近一次全量离线测试：`333 passed, 7 skipped`。真实 Provider/QQ live tests 保持默认跳过。

本阶段仍不实现：`wait` 参数、后台任务、结果异步注入、MessageBus 回传、子任务状态查询、取消/超时/并发限制、子 Agent Session 持久化及多 Agent 协作。
...
- [x] `SubagentManager` 持有运行时 Provider；模型侧任务输入仅为 `task`，运行时可额外传入非模型参数 `RequestContext`；正常 Agent 仍可通过 `AgentRunSpec.blocked_tool_names` 隐藏并拒绝指定工具调用。
...
本阶段仍不实现：子 Agent 后台任务/状态查询/取消/并发限制、子 Agent Session 持久化、主子 Agent 消息总线回传，以及 MCP 工具向子 Agent 的动态同步。
